  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const deals = mapped.filter((p) => p.originalPrice && p.originalPrice > p.price);
        setProducts(deals.length > 0 ? deals : mapped.slice(0, 12));
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          {loading ? (
            <Loader size="lg" text="Loading deals..." />
          ) : products.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-[#6B7280] text-lg">No deals available at the moment.</p>
            </div>
          ) : (
